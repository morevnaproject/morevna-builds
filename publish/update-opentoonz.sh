...
run() {
	...
	sudo $SCRIPT install_release opentoonz-master
	local DIR="$PACKET_BUILD_DIR/$PLATFORM/opentoonz-master/install_release"
	local VERSION=`cat "$DIR/version-opentoonz-master" | cut -d'-' -f 1`
	local COMMIT=`cat "$DIR/version-opentoonz-master" | cut -d'-' -f 2-`
	COMMIT="${COMMIT:0:5}"
